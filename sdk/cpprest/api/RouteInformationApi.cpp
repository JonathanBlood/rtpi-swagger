...
RouteInformationApi::RouteInformationApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<std::shared_ptr<RouteList>> RouteInformationApi::getRoutes(utility::string_t format, utility::string_t operator)
{
    
    
    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/routelistinformation");
...
responseHttpContentTypes.insert( U("application/xml") );
    ...
    {
        throw ApiException(400, U("RouteInformationApi->getRoutes does not produce any supported media type"));
    }    
...
consumeHttpContentTypes.insert( U("application/xml") );
    ...
    {
        throw ApiException(415, U("RouteInformationApi->getRoutes does not consume any supported media type"));
    }    
    ...
    .then([=](web::http::http_response response)
    {
		...
		if (response.status_code() >= 400)
		{
			throw ApiException(response.status_code()
				, U("error calling getRoutes: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling getRoutes: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
    ...
    {
        std::shared_ptr<RouteList> result(new RouteList());
