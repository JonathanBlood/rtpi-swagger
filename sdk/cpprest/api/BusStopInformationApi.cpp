...
#include "BusStopInformationApi.h"
#include "IHttpBody.h"
#include "JsonBody.h"
...
BusStopInformationApi::BusStopInformationApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<std::shared_ptr<BusStopList>> BusStopInformationApi::getBusStops(utility::string_t format, utility::string_t operator, utility::string_t stopid, utility::string_t stopname)
{
    
    
    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/busstopinformation");
...
responseHttpContentTypes.insert( U("application/xml") );
    ...
    {
        throw ApiException(400, U("BusStopInformationApi->getBusStops does not produce any supported media type"));
    }    
...
consumeHttpContentTypes.insert( U("application/xml") );
    ...
    {
        queryParams[U("stopname")] = ApiClient::parameterToString(stopname);
    ...
    {
        throw ApiException(415, U("BusStopInformationApi->getBusStops does not consume any supported media type"));
    }    
    ...
    .then([=](web::http::http_response response)
    {
		...
		if (response.status_code() >= 400)
		{
			throw ApiException(response.status_code()
				, U("error calling getBusStops: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling getBusStops: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
    ...
    {
        std::shared_ptr<BusStopList> result(new BusStopList());
