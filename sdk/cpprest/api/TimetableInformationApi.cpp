...
#include "TimetableInformationApi.h"
#include "IHttpBody.h"
#include "JsonBody.h"
...
TimetableInformationApi::TimetableInformationApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<std::shared_ptr<TimetableList>> TimetableInformationApi::getTimeTable(utility::string_t type, utility::string_t stopid, utility::string_t routeid, utility::string_t format)
{
    
    
    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/timetableinformation");
...
responseHttpContentTypes.insert( U("application/xml") );
    ...
    {
        throw ApiException(400, U("TimetableInformationApi->getTimeTable does not produce any supported media type"));
    }    
...
consumeHttpContentTypes.insert( U("application/xml") );
    ...
    {
        throw ApiException(415, U("TimetableInformationApi->getTimeTable does not consume any supported media type"));
    }    
    ...
    .then([=](web::http::http_response response)
    {
		...
		if (response.status_code() >= 400)
		{
			throw ApiException(response.status_code()
				, U("error calling getTimeTable: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling getTimeTable: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
    ...
    {
        std::shared_ptr<TimetableList> result(new TimetableList());
