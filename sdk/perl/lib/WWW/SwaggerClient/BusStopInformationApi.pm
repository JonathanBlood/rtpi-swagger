...
#
package WWW::SwaggerClient::BusStopInformationApi;
...
# @param string $stopname The stop name, only matching stops are returned (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_bus_stops' } = { 
...
sub get_bus_stops {
    ...
    # parse inputs
    my $_resource_path = '/busstopinformation';
    ...
    # query params
    if ( exists $args{'stopname'}) {
        $query_params->{'stopname'} = $self->{api_client}->to_query_value($args{'stopname'});
    }
    ...
    my $_response_object = $self->{api_client}->deserialize('BusStopList', $response);
