...
#
package WWW::SwaggerClient::RealTimeBusInformationApi;
...
# @param string $maxresults The maximum number of results, if not provided system will limit the number of results depending on server configuration (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_real_time_bus_information' } = { 
...
#
sub get_real_time_bus_information {
    my ($self, %args) = @_;

    # verify the required parameter 'stopid' is set
    unless (exists $args{'stopid'}) {
      croak("Missing the required parameter 'stopid' when calling get_real_time_bus_information");
    }

    # parse inputs
    my $_resource_path = '/realtimebusinformation';
    ...
    # query params
    if ( exists $args{'maxresults'}) {
        $query_params->{'maxresults'} = $self->{api_client}->to_query_value($args{'maxresults'});
    }
    ...
    my $_response_object = $self->{api_client}->deserialize('RealTimeInformationList', $response);
